  ...
  <div class="content-page">
    ...
    <div class="content-main">
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <!-- change -->
        <el-tab-pane label="类型1" name="second"></el-tab-pane>
        <el-tab-pane label="类型2" name="third"></el-tab-pane>
        <el-tab-pane label="类型3" name="fourth"></el-tab-pane>
      </el-tabs>
      <div class="filter-box">
        <el-form :inline="true" :model="filterForm" class="form-inline">
          <el-form-item label="数据名称">
            <el-input size="small" v-model="filterForm.name" placeholder="数据名称"></el-input>
          </el-form-item>
          <el-form-item>
            ...
            <el-button type="success" size="small" @click="exportExcel">点击导出Excel</el-button>
            <el-button type="danger" size="small" @click="getPdf()">点击导出Pdf</el-button>
          </el-form-item>
        </el-form>
      ...
      <div>
        <div class="form-table-box" id="pdfDom">
          <el-table :data="tableData" style="width: 100%" stripe id="out-table">
            ...
            <el-table-column prop="sort_order" label="数量" width="140" sortable>
              <template slot-scope="scope">
                <el-input-number
                  class="sort-width"
                  size="mini"
      ...
      <el-button type="primary" @click="savePdf">导出(pdf)</el-button>-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      exportExcel() {
        this.axios.post('excel/exportExcel', {}).then(response => {
          let res = response.data.data
          console.log(res)
          let databaseTitle=['id','name','goods_brief','sort_order','list_pic_url','is_delete']
          var newArr = new Array(res.length)
          for(let i=0;i<res.length;i++){
            newArr[i]=[]
            for(let j=0;j<databaseTitle.length;j++){
              //(j==0||j==3||j==5)?newArr[i]=newArr[i].concat([res[i][databaseTitle[j]]]):newArr[i]=newArr[i].concat([new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer(res[i][databaseTitle[j]])])
              (j==0||j==3||j==5)?newArr[i]=newArr[i].concat([new Buffer('\xEF\xBB\xBF', 'binary'),[res[i][databaseTitle[j]]]]):newArr[i]=newArr[i].concat([new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer(res[i][databaseTitle[j]])])
              //newArr[i]=newArr[i].concat([res[i][databaseTitle[j]]])
              //newArr[i]=newArr[i].concat([new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer(res[i][databaseTitle[j]])])
            }
          }

          newArr.unshift([[new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer('ID')], 
          [new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer('名称')],
          [new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer('简介')],
          [new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer('数量(sort)')],
          [new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer('图片')],
          [new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer('是否删除')]])

          let exportToCsv = function () {
            var CsvString = ''
            newArr.forEach(function (RowItem, RowIndex) {
          ...
          exportToCsv()
          /* 
var msExcelBuffer = Buffer.concat([new Buffer('\xEF\xBB\xBF', 'binary'),new Buffer('中文1,a1,b1,123\n中文2,a2,b2,456')]);
fs.writeFileSync('test.csv', msExcelBuffer);
// 直接写入 csv, 使用 excel 打开会是乱码
// fs.writeFileSync('test.csv','中文1,a1,b1,123\n中文2,a2,b2,456'); */
          this.$message({
            type: 'success',
            message: '导出成功!'
          })
